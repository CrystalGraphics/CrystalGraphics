...
| Java toolchain (compile) | 17 for 1.20.x; 8-bytecode for 1.7.10 via **jvmDowngrader** (`enableModernJavaSyntax = jvmDowngrader` in `runtime/mc/1710/gradle.properties` — Jabel is the unselected alternative there, and `core`/`platform` desugar with neither) |
...
## runtime/mc/1710 (Minecraft 1.7.10 + Forge 10.13.4)

**Plugin**: `com.gtnewhorizons.gtnhconvention` 2.0.20 (wraps RetroFuturaGradle / RFG).

- All 1.7.10-specific version pins live in `runtime/mc/1710/gradle.properties` (Minecraft, Forge, MCP channel/mappings, mixin/coremod config).
...
- *`idea.project is null` on subproject* — `gtnh.modules.ideIntegration = false` in `runtime/mc/1710/gradle.properties` suppresses the IDEA module that only works on the root project.
...
### NeoForge (`runtime/mc/modern/neoforge`)
...
Each 1.20.x loader module provides an `extractMcSources` task that unpacks the toolchain-generated
...
| Module | Trigger | Why |
|---|---|---|
| `runtime/mc/modern/neoforge` | `classes` (every normal `compileJava` / `build`) | `createMinecraftArtifacts` already ran; extraction is just a cheap unzip |
| `runtime/mc/modern/forge` | `classes` (every normal `compileJava` / `build`) | Same — sources jar is already present from ModDevGradle setup |
| `runtime/mc/modern/fabric` | IntelliJ Gradle sync (`ideaSyncTask`) — **not** on CLI builds | `genSourcesWithVineflower` (Vineflower decompilation) is an optional dev task; forcing it on `classes` would silently add 2–5 min to every fresh-clone `compileJava` |
