 ...
 * and a {@link WorldTextScaleResolver} that enforces always-MSDF rendering.</p>
 *
 ...
 *   <li><strong>Projection-aware quality</strong>: Callers can update the
 *       projected-size hint via {@link #updateProjectedSize(Matrix4f, Matrix4f, int)}
 *       to improve MSDF quality at different viewing distances.</li>
...
public class CgWorldTextRenderContext extends CgTextRenderContext {
    ...
    private final WorldTextScaleResolver worldResolver;
    ...
    public CgWorldTextRenderContext(FloatBuffer projection,
                                    int viewportWidth,
                                    int viewportHeight) {
        super(projection, new WorldTextScaleResolver());
        this.worldResolver = (WorldTextScaleResolver) getScaleResolver();
    ...
    /**
     * Updates the projected-size hint for MSDF quality/LOD decisions.
     *
     * <p>Call this before each world-text draw to provide the resolver with
     * an estimate of how large the text appears on screen. This drives only
     * atlas tier selection — layout metrics are unaffected.</p>
     *
