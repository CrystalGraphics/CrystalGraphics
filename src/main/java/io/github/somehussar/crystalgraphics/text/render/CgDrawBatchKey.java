 ...
 *       different shader programs, so they cannot share a draw call. MSDF is the
 *       classic multi-channel signed distance field. MTSDF stores that MSDF in
 *       RGB plus a true SDF in alpha, which is why it still belongs to the
 *       distance-field family but needs a dedicated reconstruction shader.</li>
 ...
 *   <li><strong>Pixel range</strong> ({@code pxRange}) — for distance-field glyphs, the
 *       distance range used during atlas generation is uploaded as the
 *       {@code u_pxRange} shader uniform. If different pages use different
 *       range values, they need separate batches.</li>
 * </ol>
 *
 * <h3>Ordering</h3>
 * <p>The key implements {@link Comparable} so batches can be sorted by mode
 * (bitmap first, then distance-field batches) for consistent draw order. Within
 * the same mode, batches are ordered by texture ID and pxRange to minimize state
 * changes while keeping related atlas pages adjacent.</p>
 *
 * <h3>Usage</h3>
